...
photon deployment list &> /dev/null || {
    ...
    echo "  photon target set -c https://172.20.0.55:443"
    echo "  photon target login --username administrator@photon.local.v --password 'P@s\$w0rd'"
    exit 1
}
...
photon -n flavor create --kind vm --name tiny-vm --cost "vm.cpu 2 COUNT, vm.memory 4 MB, vm.cost 4 COUNT"
